...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.products.update", [$product->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="subcategory_id">{{ trans('cruds.product.fields.subcategory') }}</label>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('profile') ? 'is-invalid' : '' }}" name="profile_id" id="profile_id" required>
                    @foreach($profiles as $id => $profile)
                        <option value="{{ $id }}" {{ ($product->profile ? $product->profile->id : old('profile_id')) == $id ? 'selected' : '' }}>{{ $profile }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label class="required" for="address_id">{{ trans('cruds.product.fields.address') }}</label>
                <select class="form-control select2 {{ $errors->has('address') ? 'is-invalid' : '' }}" name="address_id" id="address_id" required>
                    @foreach($addresses as $id => $address)
                        <option value="{{ $id }}" {{ ($product->address ? $product->address->id : old('address_id')) == $id ? 'selected' : '' }}>{{ $address }}</option>
                    @endforeach
                </select>
